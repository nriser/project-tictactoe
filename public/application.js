...
/***/ function(module, exports, __webpack_require__) {
	...
	var displayInput = function displayInput(lastClickedBox) {
	...
	var updateCurrentGame = function updateCurrentGame(index, value) {
	  api.updateGame(index, value).then(ui.updateGameSuccess).catch(ui.updateGameFailure);
	};
	...
	var xWinMessage = document.createElement('p');
	// Create text element for Player O win
	var oWinMessage = document.createElement('p');
	// Create text element for draw
	var drawMessage = document.createElement('p');
	...
	var displayMessage = function displayMessage() {
	  ...
	  for (var _i2 = 0; _i2 < 3; _i2++) {
	    ...
	    boxArray[_i2 * 3].textContent === playerInput[0] && boxArray[_i2 * 3 + 1].textContent === playerInput[0] && boxArray[_i2 * 3 + 2].textContent === playerInput[0] ||
	    // columns: 0, 3, 6 -> 1, 4, 7 -> 2, 5, 8
	    boxArray[_i2].textContent === playerInput[0] && boxArray[_i2 + 3].textContent === playerInput[0] && boxArray[_i2 + 6].textContent === playerInput[0] ||
	    // diagonals: 0, 4, 8 -> 1, 4, 7 (redundant) --> 2, 4, 6
	    boxArray[_i2].textContent === playerInput[0] && boxArray[4].textContent === playerInput[0] && boxArray[8 - _i2].textContent === playerInput[0]) {
	      xWinMessage.textContent = 'X wins!';
	      document.getElementById('message').appendChild(xWinMessage);
	      for (var _i3 = 0; _i3 < 9; _i3++) {
	        boxArray[_i3].disabled = true;
	      }
	    } else if (
	    // Y win combinations
	    boxArray[_i2 * 3].textContent === playerInput[1] && boxArray[_i2 * 3 + 1].textContent === playerInput[1] && boxArray[_i2 * 3 + 2].textContent === playerInput[1] || boxArray[_i2].textContent === playerInput[1] && boxArray[_i2 + 3].textContent === playerInput[1] && boxArray[_i2 + 6].textContent === playerInput[1] || boxArray[_i2].textContent === playerInput[1] && boxArray[4].textContent === playerInput[1] && boxArray[8 - _i2].textContent === playerInput[1]) {
	      oWinMessage.textContent = 'O wins!';
	      document.getElementById('message').appendChild(oWinMessage);
	      for (var _i4 = 0; _i4 < 9; _i4++) {
	        boxArray[_i4].disabled = true;
	      }
	    }
	  }
	  // Draw
	  if (playerInput.length === 9 && oWinMessage.textContent === '' && xWinMessage.textContent === '') {
	    // If 9 inputs or more, display draw message
	    // Create text element for draw
	...
	var createGame = function createGame() {
	  for (var _i6 = 0; _i6 < 9; _i6++) {
	    ...
	    boxArray[_i6].textContent = null;
	    // undisable (re-enable) each button
	    boxArray[_i6].disabled = false;
...
/***/ function(module, exports, __webpack_require__) {
	...
	var returnResults = function returnResults(response) {
	  var allGames = [];
	...
	var getGamesSuccess = function getGamesSuccess(response) {
	  ...
	  var totalWins = 0;
	  for (var j = 0; j < response.games.length; j++) {
	    for (var i = 0; i < 3; i++) {
	      if (
	      // X win combinations
	      // 0, 1, 2 -> 3, 4, 5 -> 6, 7, 8
	...
	var getGameSuccess = function getGameSuccess(response) {
	  ...
	  var currentBoard = response.game.cells;
